...
function pridifyRectangles(){
  ...
  days.forEach(function(rect) {
      switch(rect.getAttribute('fill').toUpperCase()) {
          case GH[0]: rect.setAttribute('fill', CO[3]); break; // yellow
          case GH[1]: rect.setAttribute('fill', CO[Math.floor(Math.random() * 3)]); break; // purple || pink
          case GH[2]: rect.setAttribute('fill', CO[4]); break; // green
          case GH[3]: rect.setAttribute('fill', CO[5]); break; // blue
          case GH[4]: rect.setAttribute('fill', CO[6]); break; // purple
      }
  });
}
